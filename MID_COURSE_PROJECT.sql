...
select store_id, count(inventory_id) as counts_of_inventory_items
from inventory
group by store_id
...
select count(email) as customer_email_addresses
from customer
...
select store_id, count( distinct film_id) as unique_film_titles
from inventory
group by store_id
...
select avg(amount) as average_payment, max(amount) as maximum_payment from payment 
